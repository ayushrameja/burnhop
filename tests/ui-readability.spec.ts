...
for (const viewport of viewports) {
  test(`map cards, launch, and edge HUD stay readable at ${viewport.width}×${viewport.height}`, async ({ page }) => {
    ...
    await expect(page.getByRole('group',{name:'Pistol: 12 rounds remaining, 12 round magazine capacity'})).toBeVisible();
...
test('ammo count, empty warning, and reload progress follow real shooting and keyboard reload', async ({ page }) => {
  ...
  const row=ammo.locator('.ammo-row[data-hand="main"]');
  await expect(page.getByTestId('hud-ammo')).toHaveText('12');
  await moveAim(page, 1100, 400);
  await page.mouse.down();
  await page.waitForFunction(() => window.__BURNHOP__!.snapshot().player.weapon.ammo <= 2);
  await page.mouse.up();
  await expect(row).toHaveAttribute('data-ammo-warning', 'true');
  ...
  await expect(row).toHaveAttribute('data-reloading', 'true');
  ...
  await expect(page.getByTestId('hud-ammo')).toHaveText('12');
  await expect(row).toHaveAttribute('data-reloading', 'false');
  await expect(row).toHaveAttribute('data-ammo-warning', 'false');
...
test('dual magazines remain readable at 320 pixels and reload through each independent clock',async({page})=>{
  await page.setViewportSize({width:320,height:844});await openMenu(page);await enterPractice(page);
  await page.keyboard.press('Escape');
  await page.getByRole('combobox',{name:'Practice weapon',exact:true}).selectOption('pistol');
  await page.getByRole('combobox',{name:'Second hand',exact:true}).selectOption('uzi');
  await page.getByRole('button',{name:'Resume',exact:true}).click();
  await expect(page.getByTestId('hud-ammo')).toHaveText('12');
  await expect(page.getByTestId('hud-offhand-ammo')).toHaveText('20');
  const main=page.locator('.ammo-row[data-hand="main"]'),offhand=page.locator('.ammo-row[data-hand="offhand"]');
  const [pilotBounds,ammoBounds,mainBounds,offhandBounds]=await Promise.all([box(page.locator('.pilot-hud')),box(page.getByRole('region',{name:'Ammunition'})),box(main),box(offhand)]);
  for(const bounds of [pilotBounds,ammoBounds,mainBounds,offhandBounds])withinViewport(bounds,{width:320,height:844});
  doNotOverlap(pilotBounds,ammoBounds);doNotOverlap(mainBounds,offhandBounds);
  for(const id of ['hud-ammo','hud-offhand-ammo'])expect(await page.getByTestId(id).evaluate(el=>Number.parseFloat(getComputedStyle(el).fontSize))).toBeGreaterThanOrEqual(32);
  await moveAim(page,280,330);await page.mouse.down();
  await page.waitForFunction(()=>{const p=window.__BURNHOP__!.snapshot().player;return p.weapon.ammo<10 && !!p.offhand && p.offhand.ammo<18;});
  await page.mouse.up();await page.keyboard.press('KeyR');
  await expect(main).toHaveAttribute('data-reloading','true');
  await expect(offhand).toHaveAttribute('data-reloading','false');
  await expect(offhand).toHaveAttribute('data-reloading','true');
  await expect(main).toHaveAttribute('data-reloading','false');
  await expect(page.getByTestId('hud-ammo')).toHaveText('12');
  await expect(page.getByTestId('hud-offhand-ammo')).toHaveText('20');
  await noPageOverflow(page);await page.screenshot({path:`${screenshots}/hud-dual-320x844.png`});
});
