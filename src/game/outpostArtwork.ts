import type { TerrainPolygon, Vec2 } from './types';
...
export interface TerrainArtwork extends ArtworkBounds { terrain: TerrainPolygon; index: number }
export interface ArtworkRequest extends TerrainArtwork {
  id: string; resolution: number;
}

/** Static vector scenery and procedural terrain recipes. */
export class OutpostArtwork {
  private back: CanvasPicture | undefined;
  private front: CanvasPicture | undefined;
  ...
  decoration(ctx: CanvasRenderingContext2D, foreground = false) {
    if (typeof Path2D === 'undefined') { this.paintDecoration(ctx, foreground); return; }
    const key = foreground ? 'front' : 'back';
    this[key] ??= new CanvasPicture(buffer => this.paintDecoration(buffer, foreground));
    this[key]!.draw(ctx);
  }
  private paintDecoration(ctx: CanvasRenderingContext2D, foreground = false) {
    if (foreground) {
  ...
  paintTerrain(ctx: CanvasRenderingContext2D, surface: TerrainArtwork) {
